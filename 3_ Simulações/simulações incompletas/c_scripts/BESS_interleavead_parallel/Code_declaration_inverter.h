...
// struct regular de um controlador PI
typedef struct {
	float Xref;
	float Xm;
	float erro;
...
sPI PIq   = PI_default; // PI da seção de controle de Q


// struct regular de um controlador PR
typedef struct {
float Xref;
float Xm;
float erro;
...
void Ramp(sRamp *rmp, float sample)
{
  ...
  if(rmp->rate > rmp->rising) 
    ...
    if(rmp->rate < rmp->falling) rmp->y = (rmp->t1 - rmp->t1_ant)*rmp->falling + rmp->y_ant;
      else rmp->y = rmp->uin;
