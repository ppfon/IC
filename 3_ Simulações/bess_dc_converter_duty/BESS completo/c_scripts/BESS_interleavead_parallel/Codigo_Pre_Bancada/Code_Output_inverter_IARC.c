...
if(count == PRD)
{
  ...
  if(control_enable == 1)
  {
    ...
    PRf_alfa.Xref = (PLL.Valfa_in*(P_control) - Q_control*PLL.Vbeta_in)/((PLL.Valfa_in*PLL.Valfa_in + PLL.Vbeta_in*PLL.Vbeta_in + 1e-2)^2);
    PRf_beta.Xref = (PLL.Vbeta_in*(P_control) + Q_control*PLL.Valfa_in)/((PLL.Valfa_in*PLL.Valfa_in + PLL.Vbeta_in*PLL.Vbeta_in + 1e-2))^2;
